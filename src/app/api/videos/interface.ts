export interface IResponseGetVideoInf {
  status_code: number;
  min_cursor: number;
  max_cursor: number;
  has_more: number;
...
export interface OriginalClientText {
  markup_text: string;
  text_extra: OriginalClientTextTextExtra[] | null;
}

export interface OriginalClientTextTextExtra {
  type: number;
  hashtag_name: string;
  is_commerce: boolean;
  sub_type: number;
...
export interface AwemeListTextExtra {
  start: number;
  end: number;
  user_id: string;
  type: number;
